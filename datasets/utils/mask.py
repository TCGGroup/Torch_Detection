...
def mask_parse(annotation, gt_masks, gt_mask_polys, gt_poly_lens, dataset):
    ...
    Args:
        ...
        gt_masks (list): The list of ground truth masks, each element
            in this list is will generate a ground truth binary mask
        gt_mask_polys (list): The list of ground truth poly coordinates,
            each element in this list is a couple of coordinates for each
            point in the polygons.
        gt_poly_lens (list): Each element in this list is number of points
            in the polygons.
        dataset (:obj:dataset): The dataset object, e.g. COCO dataset object.
    """

    # annToMask: first use `annToRLE` to compress polygons, and then use `decode`
    # convert the compressed polygon into a binary mask.
...
def mask_visualize(img_array, masks, inds,
    ...
    Args:
        ...
        inds (ndarray of bool or `[]`): The `inds` get from `:func:bbox_visualize`
...
# mask resize
##############################################
def mask_resize(mask, scale_factor=None, return_scale=False, interpolation='nearest'):
    """
    Resize the binary mask according to the `scale_factor` that used to resize the image.
    Because `mask` is a binary image, so this function is the Same as `:func:img_resize`

    Args:
        mask (ndarray, np.uint8): Binary image for object, the shape of `mask` is `[h, w]`
    ...
    resized_mask = img_resize(
        mask, scale_factor=scale_factor, return_scale=return_scale, interpolation=interpolation)
    return resized_mask
