...
def mask_parse(annotation, gt_masks, gt_mask_polys, gt_poly_lens, dataset):
    ...
    # annToMask: first use `annToRLE` to compress polygons,
    # and then use `decode` convert the compressed polygon into a binary mask.
...
# mask resize
##############################################
def mask_resize(mask, scale_factor=None, return_scale=False,
                interpolation='nearest'):
    """
    Resize the binary mask according to the `scale_factor` that used to
    resize the image.
    Because `mask` is a binary image, so this function is the Same as
    `:func:img_resize`
    ...
    resized_mask = img_resize(
        mask, scale_factor=scale_factor, return_scale=return_scale,
